...
class Obatkosong extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $header['title'] = "Admin Obat Kosong";
        $this->load->view('shared/admin_header', $header);

        $data['currentPage'] = 'obatkosong';
        $this->load->view('shared/admin_sidebar', $data);
        $this->load->view('shared/admin_topbar');
        $this->load->view('pages/obatkosong', $data);
        $this->load->view('shared/admin_footer', $data);
    }
    function obatkosong_fetch()
    {
        $query = "SELECT o.*,k.nama_kategori_obat,s.nama_satuan_obat FROM obat o inner join kategori_obat k on k.id_kategori = o.id_kategori inner join satuan_obat s on s.id_satuan = o.id_satuan Where o.stok = 0";
