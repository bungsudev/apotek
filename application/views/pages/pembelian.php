...
<div class="container-fluid" id="adminObat">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-2">
        <h1 class="h3 mb-2 text-gray-800">Pembelian Obat</h1>
        <a href="<?= base_url() ?>pembelian/add_pembelian" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-plus fa-sm text-white "></i> Buat Pembelian Obat </a>
    </div>
    <?php
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive" id="dataTablePembelian">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#data-table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('pembelian/pembelian_fetch_serverside')?>",
                "type": "POST",
                error(e){
                    console.log(e)
                }
        ...
        function getListPembelian() {
            $('.loading').show();
            $.ajax({
                url: "<?php echo base_url(); ?>pembelian/pembelian_fetch",
                method: "POST",
                success: function(data) {
                    $('#dataTablePembelian').html(data);
                    $("#dataTable").DataTable();
                    $('.loading').hide();
                },
                error: function(err) {
        ...
        $(document).on('click', ".addObatSubmit", function() {
            ...
            $.ajax({
                url: '<?= base_url() ?>pembelian/obat_add',
                cache: false,
                contentType: false,
                processData: false,
                data: form_data,
        ...
        $(document).on('click', ".activeObatButton", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $('.loading').show()
                    $.ajax({
                        url: '<?= base_url() ?>pembelian/obat_setactive',
                        data: {
                            id_obat: id_obat,
                            is_active: active
                        },
        ...
        $('#buttonAddNew').click(function() {
            $('#formObat').trigger("reset");
            $('#previewImageObat').attr('src', '<?= base_url() ?>assets/images/admin/pembelian/obat/defaultObat.jpg');
            $('.custom-file-input').next('.custom-file-label').addClass('selected').html('')
            $('#is_active').prop('checked', true);
            $('#pembelianObat #pembelianObatLabel').html('Add New Obat ')
            $('#pembelianObat #addObat').html('Add New Obat ')
            $('#addObat').addClass('addObatSubmit')
            $('#addObat').removeClass('editObatSubmit')
            $('#stok').attr('readonly', false)
            $('#pembelianObat').modal('show');
        })
        $(document).on('click', ".editObatButton", function() {
            const id_obat = $(this).data('id_obat');
            $.ajax({
                url: '<?= base_url() ?>pembelian/obat_selected',
                data: {
                    id_obat: id_obat
                },
                method: 'POST',
                ...
                success: function(data) {
                    ...
                    $('#previewImageObat').attr('src', '<?= base_url() ?>assets/images/admin/pembelian/obat/' + data['image']);
                    if (data['is_active'] == '1') {
                        $("#is_active").prop("checked", true);
                    } else {
                        $("#is_active").prop("checked", false);
                    }
                    $('#pembelianObat #pembelianObatLabel').html('Edit Obat ')
                    $('#pembelianObat #addObat').html('Edit Obat ')
        ...
        $(document).on('click', ".editObatSubmit", function() {
            ...
            var form_data = new FormData($('#formObat')[0])
            $.ajax({
                url: '<?= base_url() ?>pembelian/obat_update',
                cache: false,
                contentType: false,
                processData: false,
                data: form_data,
